    ...
    >
      <div 
        className={`relative overflow-hidden transition-all duration-300 ease-in-out shrink-0 flex items-center ${
          isCollapsed ? 'w-7 h-7 md:w-8 md:h-8' : 'w-[84px] h-7 md:w-[96px] md:h-8'
        }`}
      >
        <img 
          src="/tt-p.png" 
          alt="BodhSight Logo" 
          className="absolute left-0 top-0 h-full w-auto max-w-none object-cover object-left drop-shadow-sm dark:drop-shadow-[0_0_12px_rgba(255,255,255,0.4)] transition-all" 
        />
      </div>
    </div>
  );
