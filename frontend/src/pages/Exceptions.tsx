  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="space-y-6">
        {exceptions.map(exc => (
          <div key={exc.id} className="bg-[#0B1120] rounded-xl border border-red-200 shadow-sm overflow-hidden flex flex-col md:flex-row">
            ...
            <div className="p-6 md:w-2/3 flex flex-col justify-between">
              <div className="space-y-4">
                <div>
                  <h4 className="text-sm font-bold text-white mb-1">Agent 10 Analysis</h4>
                  <p className="text-sm text-slate-300 leading-relaxed bg-slate-900/40 p-3 rounded-lg border border-slate-800/60">
                    {exc.explanation}
                  </p>
                </div>
              </div>

              <div className="mt-6 pt-4 border-t border-slate-800/60 flex flex-col sm:flex-row gap-3 items-center justify-between">
                <button 
                  onClick={() => setSelectedException(exc)}
                  className="inline-flex items-center gap-2 text-indigo-400 font-semibold text-sm hover:underline"
                >
      ...
      {selectedException && (
        <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#0B1120] rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden border border-slate-800/60">
            <div className="px-6 py-4 border-b border-slate-800/60 flex justify-between items-center bg-indigo-500/10/60">
            ...
            <div className="p-6 space-y-6">
              <div>
                ...
                <h4 className="text-xl font-bold text-white mt-2">{selectedException.title}</h4>
              </div>
