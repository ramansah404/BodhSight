...
/**
 * Login page ΓÇö institutional sign-in with database-backed authentication.
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        if (res.success) {
          setSuccess("Account created! Signing you inΓÇª");
          setTimeout(() => {
            startSession(
              res.role ?? roleCode,
              selectedRole.label,
      ...
      >
        ...
        <div className="md:col-span-5 bg-surface-secondary/50 p-8 md:p-12 border-r border-border flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="text-[11px] text-secondary font-medium pt-8 mt-4 border-t border-border">
            bcrypt-encrypted credentials ΓÇó PostgreSQL-backed ΓÇó RBAC session
          </div>
        </div>
        ...
        <div className="md:col-span-7 p-8 md:p-12 flex flex-col justify-center bg-surface">
          ...
          <form onSubmit={handleAuthSubmit} className="space-y-5">
            ...
            <div className={`grid grid-cols-1 gap-5 ${isSignUp && needsDept ? "sm:grid-cols-2" : ""}`}>
              ...
              {/* Department ΓÇö only shown for HOD/Faculty and during Sign Up */}
              {isSignUp && needsDept && (
                <div>
                  <label className="block text-xs font-semibold text-secondary uppercase tracking-wider mb-2">
                    Department
            ...
            {/* Confirm Password ΓÇö Sign Up only */}
            <AnimatePresence>
              {isSignUp && (
                <motion.div
                  key="confirm-field"
            ...
            >
              ...
              {loading
                ? (isSignUp ? "Creating AccountΓÇª" : "Verifying CredentialsΓÇª")
                : isSignUp
                  ...
                  {forgotStep === 2 && (
                    <motion.form key="s2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} onSubmit={handleUpdatePassword} className="space-y-5">
                      ...
                      <div>
                        ...
                        <input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="ΓÇóΓÇóΓÇóΓÇóΓÇóΓÇóΓÇóΓÇó" className="w-full px-4 py-2.5 bg-background border border-border rounded-xl text-sm text-primary focus:border-indigo-500 focus:outline-none shadow-sm" required />
                      </div>
                      <button type="submit" disabled={forgotLoading} className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-sm font-bold transition-all cursor-pointer shadow-sm">
