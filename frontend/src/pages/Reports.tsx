...
export default function Reports() {
  ...
  const handleExportExcel = (rep: ReportItem) => {
    const exportData = [
      ...
      {},
      { "Metric": "Students Evaluated", "Value": metrics?.students_evaluated ?? "ΓÇö" },
      { "Metric": "Pass Rate (%)", "Value": metrics?.pass_rate?.toFixed(1) ?? "ΓÇö" },
      { "Metric": "Avg Marks", "Value": metrics?.average_marks?.toFixed(1) ?? "ΓÇö" },
      { "Metric": "Active Anomalies", "Value": metrics?.active_anomalies ?? "ΓÇö" },
      {},
  ...
  const handleExportPDF = (rep: ReportItem) => {
    const paragraphs = [
      ...
      `Students Evaluated: ${metrics?.students_evaluated ?? "ΓÇö"}`,
      `Institutional Pass Rate: ${metrics?.pass_rate?.toFixed(1) ?? "ΓÇö"}%`,
      `Average Marks: ${metrics?.average_marks?.toFixed(1) ?? "ΓÇö"}`,
      `Active Problems: ${metrics?.active_anomalies ?? "ΓÇö"}`,
      `Data Trust Score: ${metrics?.data_trust_score ?? "ΓÇö"}/100`,
    ];
    const tableData = departments.length > 0 ? [
      ["Department", "Pass Rate (%)", "Status"],
      ...departments.map(d => [d.department_code, String(d.pass_rate ?? "ΓÇö"), d.status ?? "ΓÇö"])
    ] : null;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6" id="reports-content">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Students Evaluated", value: metricsLoading ? "ΓÇª" : (metrics?.students_evaluated?.toLocaleString() ?? "ΓÇö"), icon: <Users size={16} />, color: "indigo" },
          { label: "Pass Rate", value: metricsLoading ? "ΓÇª" : (metrics?.pass_rate != null ? `${metrics.pass_rate.toFixed(1)}%` : "ΓÇö"), icon: <TrendingUp size={16} />, color: "emerald" },
          { label: "Avg Marks", value: metricsLoading ? "ΓÇª" : (metrics?.average_marks?.toFixed(1) ?? "ΓÇö"), icon: <BookOpen size={16} />, color: "blue" },
          { label: "Active Anomalies", value: metricsLoading ? "ΓÇª" : ((metrics?.active_anomalies ?? metrics?.significant_deviations) ?? "ΓÇö"), icon: <AlertTriangle size={16} />, color: "rose" },
        ].map((kpi) => (
            ...
            <ResponsiveContainer>
              <BarChart data={deptChartData} margin={{ top: 10, right: 10, left: -10, bottom: 10 }}>
                ...
                <Tooltip
                  ...
                  formatter={(val: any) => [`${val.toFixed(1)}%`, "Pass Rate"]}
                />
            ...
            <div key={rep.id} className="bg-surface-secondary rounded-2xl p-5 border border-border flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-indigo-300 dark:hover:border-indigo-700 transition-all">
              <div className="space-y-1 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-xs text-secondary/60 font-medium">ΓÇó {rep.size}</span>
            ...
            <div className="p-8 space-y-6">
                ...
                {metricsLoading ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-2xl p-4 text-center">
                      ...
                      <div className="text-2xl font-black text-indigo-600 dark:text-indigo-400">{metrics?.students_evaluated?.toLocaleString() ?? "ΓÇö"}</div>
                    ...
                    <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 text-center">
                      ...
                      <div className="text-2xl font-black text-emerald-600 dark:text-emerald-400">{metrics?.pass_rate?.toFixed(1) ?? "ΓÇö"}%</div>
                    ...
                    <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-4 text-center">
                      ...
                      <div className="text-2xl font-black text-blue-600 dark:text-blue-400">{metrics?.average_marks?.toFixed(1) ?? "ΓÇö"}</div>
                    ...
                    <div className="bg-rose-500/10 border border-rose-500/20 rounded-2xl p-4 text-center">
                      ...
                      <div className="text-2xl font-black text-rose-600 dark:text-rose-400">{metrics?.active_anomalies ?? metrics?.significant_deviations ?? "ΓÇö"}</div>
                  ...
                  <div className="space-y-2">
                    {departments.map((d) => (
                      <div key={d.department_code} className="flex items-center gap-3">
                        ...
                        <div className="w-14 text-xs font-bold text-primary text-right shrink-0">{d.pass_rate?.toFixed(1) ?? "ΓÇö"}%</div>
                        <div className={`px-2 py-0.5 rounded-full text-[10px] font-bold shrink-0 ${
                          ...
                          {d.status === "INTERVENTION_REQUIRED" ? "ΓÜá CRITICAL" : d.status === "MONITORING" ? "ΓÜí WATCH" : "Γ£ô OK"}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="bg-surface-secondary rounded-2xl p-5 border border-border space-y-2">
                ...
                <div className="grid grid-cols-2 gap-y-2 text-sm">
                  ...
                  <div className="font-bold text-emerald-600 dark:text-emerald-400">{metrics?.data_trust_score ?? "ΓÇö"}/100</div>
                </div>
              </div>
